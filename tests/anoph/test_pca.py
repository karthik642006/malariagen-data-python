...
def test_pca_exclude_samples(fixture, api: AnophelesPca):
    ...
    # Check exclusions.
    assert len(pca_df.loc[pca_df["sample_id"].isin(exclude_samples)]) == 0
...
def test_pca_fit_exclude_samples(fixture, api: AnophelesPca):
    ...
    assert len(pca_df.loc[pca_df["sample_id"].isin(exclude_samples)]) == n_samples_excluded
    assert (
        len(pca_df.loc[pca_df["sample_id"].isin(exclude_samples) & ~pca_df["pca_fit"]])
        == n_samples_excluded
    )
